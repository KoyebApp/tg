...
let handler = async ({ m, bot, usedPrefix, command, text }) => {
   try {
      ...
      if (!textAfterCommand) {
         throw tee; // This will throw an error with the message 'tee'
      }
      ...
      // Handle the case when no text is provided after the command
      console.error('Error occurred:', error);
      bot.sendMessage(m.chat.id, error);  // Send the error message to the user
   }
};
